...
<?php
	...
	$feeds = array(
//		'http://feeds.feedburner.com/vicchi-geoblog',
	...
	$yql = 'select * from html where url='.
	       '"http://www.vicchi.org/speaking/"'.
       	   ' and xpath="//div[@class=\'entry\']//ul" limit 1';
	       //' and xpath="//div[@class=\'entry\']//ul" limit 3';
	...
	function renderFeeds($url)
	{
		...
		if ($x->query->results->item)
		{
			foreach ($x->query->results->item as $i)
			{
				...
				if ($i->category && strstr ($i->category->content, 'clippings') && $mencount < 5)
				{
				...
				// for interviews and geo articles, add to the articles section 
//				elseif (strstr($i->link,'ygeoblog') && $count < 5)
				elseif ($i->category && strstr ($i->category->content, 'article') && $count < 5)
				{
